...
- в ОС Windows 7 и старше: папка ``C:\Users\<имя пользователя>\.qgis2\python\plugins``;
- в macOS: папка ``<имя пользователя>\.qgis2\python\plugins``. Папка .qgis2 является скрытой папкой.
